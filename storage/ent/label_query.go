...
func (lq *LabelQuery) QueryOperation() *OperationQuery {
	query := (&OperationClient{config: lq.config}).Query()
...
func (lq *LabelQuery) Clone() *LabelQuery {
	...
	return &LabelQuery{
		...
		withOperation: lq.withOperation.Clone(),
...
func (lq *LabelQuery) WithOperation(opts ...func(*OperationQuery)) *LabelQuery {
	query := (&OperationClient{config: lq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	lq.withOperation = query
	return lq
}
...
func (lq *LabelQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Label, error) {
	...
	if query := lq.withOperation; query != nil {
		if err := lq.loadOperation(ctx, query, nodes,
			func(n *Label) { n.Edges.Operation = []*Operation{} },
			func(n *Label, e *Operation) { n.Edges.Operation = append(n.Edges.Operation, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (lq *LabelQuery) loadOperation(ctx context.Context, query *OperationQuery, nodes []*Label, init func(*Label), assign func(*Label, *Operation)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(label.OperationTable)
		s.Join(joinT).On(s.C(operation.FieldID), joinT.C(label.OperationPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(label.OperationPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(label.OperationPrimaryKey[1]))
	...
	neighbors, err := withInterceptors[[]*Operation](ctx, query, qr, query.inters)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "operation" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
